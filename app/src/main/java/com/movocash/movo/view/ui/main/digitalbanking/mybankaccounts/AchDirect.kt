...
import com.movocash.movo.databinding.FragmentAchDirectBinding
...
class AchDirect : BaseFragment(), View.OnClickListener, BottomSheetSelectorFragment.ISelectListener, IFrequencyList, BottomSheetDatePickerFragment.ISelectDobListener, ICardClickListener, IInfoListener {

    lateinit var binding: FragmentAchDirectBinding
    ...
    companion object {
        lateinit var instance: AchDirect
        var model: CustomEditScheduledTransferModel? = null

        fun newInstance(obj: CustomEditScheduledTransferModel?): AchDirect{
            instance = AchDirect()
            model = obj
            return instance
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_ach_direct, container, false)
    ...
    private fun setBankData() {
        var bankNum = ""
        if (model!!.fromAccount!!.length > 5) {
            bankNum = "*" + model!!.fromAccount!!.substring(model!!.fromAccount!!.length - 4)
        }
        val data = "${model!!.toAccount}${bankNum}"
        binding.bankNum = data
        ActivityBase.activity.showToastMessage(data.toString())
        binding.isBankSet = true
        bankSrNo = model!!.fromAccount!!
    }
