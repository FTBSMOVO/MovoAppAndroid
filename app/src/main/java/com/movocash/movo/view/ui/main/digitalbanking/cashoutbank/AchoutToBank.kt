...
import com.movocash.movo.databinding.FragmentAchoutToBankBinding
import com.movocash.movo.utilities.Constants.CARD_TO_BANK_TRANSFER
...
class AchoutToBank : BaseFragment(), View.OnClickListener, BottomSheetSelectorFragment.ISelectListener, IFrequencyList, BottomSheetDatePickerFragment.ISelectDobListener, ICardClickListener, IInfoListener {

    lateinit var binding: FragmentAchoutToBankBinding
    ...
    companion object {
        lateinit var instance: AchoutToBank
        var model: GetBankAccountsResponseModel.Account? = null

        fun newInstance(obj: GetBankAccountsResponseModel.Account?): AchoutToBank {
            instance = AchoutToBank()
            model = obj
            return instance
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_achout_to_bank, container, false)
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.llSendTo -> {
                ...
                //ActivityBase.activity.showToastMessage("ButtonClicked")

                if (isBankFound) {
                    if (bankList.size > 0) {
                        //ActivityBase.activity.showToastMessage("1 bank found")
                        showBankBottomSheet()
                    }
                } else {
    ...
    private fun setBankData(position: Int) {
        var bankNum = ""
        if (model!!.accountNumber!!.length > 5) {
    ...
    private fun setBankData1() {
        var bankNum = ""
        if (model!!.accountNumber!!.length > 5) {
            bankNum = "*" + model!!.accountNumber!!.substring(model!!.accountNumber!!.length - 4)
        }
        val data = "${model!!.accountNickname}${bankNum}"
        binding.bankNum = data
        binding.isBankSet = true
        bankSrNo = model!!.accountSrNo!!
    }
    ...
    private fun proceedToEdit() {
       ...
       // bankViewModel.cashToBankTransfer(CashOutToBankRequestModel(referenceId,"", bankSrNo, binding.etAmount.text.toString().toDouble(), binding.etComments.text.toString(), "", ""))
    ...
    private fun proceedToConfirmation() {
        ...
        addFragment(R.id.mainContainer, AchOutToBankDetails.newInstance(obj), "CashOutBankDetailFragment")
    }
