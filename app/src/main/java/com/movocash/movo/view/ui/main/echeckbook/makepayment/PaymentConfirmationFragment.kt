...
class PaymentConfirmationFragment : BaseFragment(), View.OnClickListener, IInfoListener {

    lateinit var binding: FragmentPaymentConfirmationBinding
    private lateinit var viewModel: ECheckBookViewModel
    private lateinit var commonViewModel: CommonViewModel

    companion object {
        lateinit var instance: PaymentConfirmationFragment
        ...
        fun newInstance(obj1: CustomPaymentDetailModel, obj2: MakeUpdatePaymentRequestModel, isSide: Boolean, amount:Double): PaymentConfirmationFragment {
            instance = PaymentConfirmationFragment()
            customObj = obj1
            requestObj = obj2
            isFromSide = isSide
        ...
        viewModel.makeUpdatePaymentResponseModel.observe(viewLifecycleOwner, Observer {
            it?.let { obj ->
                if (obj?.data != null && obj.data!!.responseDesc != null && !TextUtils.isEmpty(obj.data!!.responseDesc)) {
                    ...
                    addFragment(R.id.mainContainer, MakePaymentReciptFragment.newInstance(customObj, requestObj, MakePaymentFragment.isFromSide,primaryAmount), "PaymentConfirmationFragment")
                }
