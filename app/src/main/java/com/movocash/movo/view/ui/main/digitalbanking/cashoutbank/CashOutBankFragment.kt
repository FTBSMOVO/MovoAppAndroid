...
class CashOutBankFragment : BaseFragment(), View.OnClickListener, BottomSheetSelectorFragment.ISelectListener, IFrequencyList, BottomSheetDatePickerFragment.ISelectDobListener, ICardClickListener, IInfoListener {

    lateinit var binding: FragmentCashOutBankBinding
    ...
    companion object {
        lateinit var instance: CashOutBankFragment
        var model: CustomEditScheduledTransferModel? = null

        fun newInstance(obj: CustomEditScheduledTransferModel?): CashOutBankFragment {
            instance = CashOutBankFragment()
            model = obj
            return instance
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_cash_out_bank, container, false)
    ...
    private fun checkAndSetBanks() {
        ...
        if (bankModel != null) {
            if (bankModel.data != null && bankModel.data!!.accounts != null && bankModel.data!!.accounts!!.size > 0) {
                ...
                plaidList = bankList.filter { it.achType==1 } as ArrayList<GetBankAccountsResponseModel.Account>
                bankList = plaidList
    ...
    private fun proceedToEdit() {
        ...
        bankViewModel.cashToBankTransfer(CashOutToBankRequestModel(referenceId,model!!.transferId, bankSrNo, binding.etAmount.text.toString().toDouble(), binding.etComments.text.toString(), model!!.frequencyType, model!!.date))
    ...
    private fun proceedToConfirmation() {
        ...
        addFragment(R.id.mainContainer, CashOutBankDetailFragment.newInstance(obj), "CashOutBankDetailFragment")
    }
    ...
    private fun showBankBottomSheet() {
        ...
        if (myBankList.size > 0) {
            ...
            //plaidList = myBankList.filter { it.achType==1 } as ArrayList<GetBankAccountsResponseModel.Account>
